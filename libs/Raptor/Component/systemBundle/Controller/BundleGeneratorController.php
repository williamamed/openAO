...
class BundleGeneratorController extends Controller {
    ...
    public function createBundleAction($request) {
        ...
        return $this->show("El componente fue creado correctamente", true, Controller::INFO, $resp);
    }
    ...
    public function createBundleDirectory($vendor, $bundleName, &$messages) {
        ...
        mkdir($src . '/' . $vendor . '/' . $bundleName . '/Manifest');
    ...
    public function deleteBundleAction($request) {
        ...
        return $this->show("El componente $name fue eliminado correctamente");
    }
    ...
    public function checkExportAction() {
        ...
        if(isset($options[$this->app->request()->post('name')])){
            ...
            if(count($install)>0){
                return $this->show("Por favor espero, estamos generando el componente");
            }else{
                return $this->show("El componente que intenta exportar no contiene un manifiesto de instalación, por favor cree uno.",true,  Controller::ERROR);
            }
        }
        return $this->show("No pudimos encontrar el componente especificado",true,  Controller::ERROR);
    }
