...
Ext.define('GestUser.controller.Users', {
        ...
        for (var i = 0, cant = this.getUserStore().count(); i < cant; i++) {
            if (this.getUserStore().getAt(i).get('icon'))
                var tip = Ext.create('Ext.tip.ToolTip', {
                    ...
                    html: "<img src='" + Raptor.getBundleResource('security_photos/' + this.getUserStore().getAt(i).get('icon')) + "' width='100'>",
                    trackMouse: true,
    ...
    onListEdit: function (editor, e) {
        ...
        Ext.Ajax.request({
            ...
            failure: function (response, opts) {
                Raptor.msg.show(3, 'No se pudo realizar la acción: código de respuesta ' + response.status);
            },
            scope: this
        });
    ...
    onTreeSelect: function (me, model) {
        Interactive.show('syn.user.selectestructure');
        this.getUserlist().getStore().load();
        this.onListDeSelect();
    ...
    onRender: function () {
        ...
        Interactive.show('syn.user.welcome');
    ...
    onLoadRol: function (store) {
        var roles = this.getUserlist().getSelectionModel().getLastSelected().data.idRol;
        for (var i = 0; i < roles.length; i++) {
            var index = store.findExact('id', roles[i].idRol);
            console.debug(roles,index,roles[i].idRol)
            if (index > -1) {
    ...
    insertUser: function (form, url) {
        ...
        form.submit({
            ...
            success: function (formBasic, action) {
                ...
                this.getUserStore().load();
                Interactive.show('syn.user.asignrol');
            },
        ...
        this.columns = [{
                ...
                renderer: function(value, meta, model, row, col, store, view) {
                    if (value) {
                        ...
                        return "<img id='id_" + value + "' src='" + Raptor.getBundleResource('security_photos/' + value) + "' width='20' height='20'>";
                    } else
                    ...
                    if (!value || value.length===0) {
                        ...
                        if (value.length > 1) {
                            ...
                            for(var i = 0; i < value.length; i++){
                                texto+=value[i].rolename+'<br>';
                            }
                            return "<b style='color:#3d4ffb' data-qtip='<b>Roles:</b><br />"+texto+"'>" + value[0].rolename + "...</b>";
                        }else{
                            return "<b style='color:#3d4ffb'>" + value[0].rolename + "</b>";
                        }
                    }
                },
                flex: 1
