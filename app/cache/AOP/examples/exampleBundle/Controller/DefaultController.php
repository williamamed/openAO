...
use Raptor2\SyntarsusBundle\Manager\SecurityManager;
/**
 ...
 */
class DefaultController__AopProxied extends \Raptor\Bundle\Controller\Controller {
     ...
     */
    public function indexAction($request, $response, $route, SecurityManager $security=NULL) {
        if($this->app)
            echo "si";
        /**$import = $this->getStore()
            ->getImporter()
            ->createIfNotExist(true);
    $result = $import->import(__DIR__ . '/../../../Raptor2/SyntarsusBundle/Installer/data/security.php');
    

    if ($result)
        echo "El modulo de seguridad Syntarsus fue instalado correctamente!!";*/
    
        if($security)
            echo 'SecurityManager Inyectado<br>';
        
        if($this->word)
            echo 'word Inyectado<br>';
    ...
    public function exportDataAction() {
        if ($this->getStoreManager()->getConnection()->getSchemaManager()->tablesExist(array('persona'))) {
            $this->getStore()
                    ...
                    ->save('C:\wamp\www\Raptor3\openAO\src\examples\exampleBundle\Controller' . '/exportExample.php');

            return file_get_contents('C:\wamp\www\Raptor3\openAO\src\examples\exampleBundle\Controller' . '/exportExample.php');
        }
    }
    ...
    public function exportZipAction() {
         ...
         * of this bundle 
         */
        $zip->create('C:\wamp\www\Raptor3\openAO\src\examples\exampleBundle\Controller');
...
include_once AOP_CACHE_DIR . '/_proxies/examples\\exampleBundle\\Controller\\DefaultController.php';
